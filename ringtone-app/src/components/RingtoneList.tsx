...
const RingtoneList: React.FC<RingtoneListProps> = ({ ringtones, onRingtonesUpdated, onEditRingtone, onDeleteLocalRingtone }) => {
  ...
  if (ringtones.length === 0 && backendRingtones.length === 0 && !isLoading) {
    return (
  ...
  return (
    <div className="ringtone-list">
      ...
      {groupedBackendRingtones.mp3.length > 0 && (
        <div className="ringtone-section">
          <h3>🎵 MP3 Ringtones ({groupedBackendRingtones.mp3.length})</h3>
          {groupedBackendRingtones.mp3.map((ringtone) => renderRingtoneItem(ringtone, false))}
        </div>
      )}

      {/* WAV Ringtones Section */}
      {groupedBackendRingtones.wav.length > 0 && (
        <div className="ringtone-section">
          <h3>🎶 WAV Ringtones ({groupedBackendRingtones.wav.length})</h3>
          {groupedBackendRingtones.wav.map((ringtone) => renderRingtoneItem(ringtone, false))}
        </div>
      )}
      ...
      {ringtones.length > 0 && (
        <div className="ringtone-section">
          <h3>🆕 All Created Ringtones ({ringtones.length})</h3>
          {ringtones.map((ringtone) => renderRingtoneItem(ringtone, true))}
        </div>
      )}
